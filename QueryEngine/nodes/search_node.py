...
class FirstSearchNode(BaseNode):
    ...
    def run(self, input_data: Any, **kwargs) -> Dict[str, str]:
        ...
        try:
            if not self.validate_input(input_data):
                raise ValueError("Input data format error, requires title and content fields")
    ...
    def process_output(self, output: str) -> Dict[str, str]:
        ...
        try:
            ...
            self.log_error(f"Processing output failed: {str(e)}")
            # Return default query
            return self._get_default_search_query()
...
class ReflectionNode(BaseNode):
    ...
    def run(self, input_data: Any, **kwargs) -> Dict[str, str]:
        ...
        try:
            if not self.validate_input(input_data):
                raise ValueError("Input data format error, requires title, content, and paragraph_latest_state fields")
