"""
Public Opinion Search Toolset (Tavily) Designed for AI Agents

Version: 1.5
Last Updated: 2025-08-22

This script decomposes complex Tavily search capabilities into a series of focused,
minimal-parameter independent tools designed for AI Agent invocation. Agents only
need to select the appropriate tool based on task intent, without understanding complex
parameter combinations. All tools default to searching "news" (topic='general').

New Features:
- Added `basic_search_news` tool for standard, general-purpose news search.
...
- deep_search_news: Perform the most comprehensive deep analysis on a topic.
- search_news_last_24_hours: Get the latest updates within 24 hours.
...
# Make sure Tavily library is installed before running: pip install tavily-python
try:
...
@dataclass
class ImageResult:
    """Image search result dataclass"""
    url: str
    description: Optional[str] = None

@dataclass
class TavilyResponse:
    """Encapsulates the complete Tavily API response for passing between tools"""
    query: str
...
class TavilyNewsAgency:
    """
    A client containing various specialized public opinion news search tools.
    Each public method is designed as a standalone tool for AI Agent invocation.
    """
    ...
    def basic_search_news(self, query: str, max_results: int = 7, **kwargs) -> TavilyResponse:
        """
        [Tool] Basic News Search: Execute a standard, fast news search.
        This is the most commonly used general search tool, suitable when unsure of specific search needs.
        Agent can provide search query and optional max results count.
        """
    ...
    def deep_search_news(self, query: str) -> TavilyResponse:
        """
        [Tool] Deep News Analysis: Perform the most comprehensive and in-depth search on a topic.
        Returns AI-generated "advanced" detailed summary answer and up to 20 most relevant news results.
        Suitable for scenarios requiring comprehensive understanding of event background.
    ...
    def search_news_last_24_hours(self, query: str) -> TavilyResponse:
        """
        [Tool] Search News in Last 24 Hours: Get the latest updates on a topic.
        This tool specifically searches for news published in the past 24 hours.
    ...
    def search_news_last_week(self, query: str) -> TavilyResponse:
        """
        [Tool] Search This Week's News: Get major news reports from the past week on a topic.
    ...
    def search_images_for_news(self, query: str) -> TavilyResponse:
        """
        [Tool] Search News Images: Search for images related to a news topic.
        This tool returns image links and descriptions, suitable for scenarios
        requiring images to accompany reports or articles.
        Agent only needs to provide search query.
        """
    ...
    def search_news_by_date(self, query: str, start_date: str, end_date: str) -> TavilyResponse:
        """
        [Tool] Search News by Date Range: Search news within a specific historical timeframe.
        This is the only tool that requires Agent to provide detailed time parameters.
        Suitable for analyzing specific historical events.
        Agent needs to provide query, start_date, and end_date, all in 'YYYY-MM-DD' format.
        """
...
def print_response_summary(response: TavilyResponse):
    """Simplified print function for displaying test results, now shows publication date"""
...
if __name__ == "__main__":
    # Before running, ensure TAVILY_API_KEY environment variable is set
    
    try:
        # Initialize the "News Agency" client, it contains all tools internally
        agency = TavilyNewsAgency()

        # Scenario 1: Agent performs a routine, quick search
        response1 = agency.basic_search_news(query="奥运会最新赛况", max_results=5)
        print_response_summary(response1)

        # Scenario 2: Agent needs comprehensive background on "global chip technology competition"
        ...
        # Scenario 5: Agent needs news images on "Webb Space Telescope"
        response5 = agency.search_images_for_news(query="韦伯太空望远镜最新发现")
        print_response_summary(response5)

        # Scenario 6: Agent needs to research news on "AI regulations" in Q1 2025
