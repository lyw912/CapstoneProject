...
class DeepSearchAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        # LangGraph subgraph (lazy initialization to avoid loading during import)
        self._query_graph = None

        logger.info(f"Query Agent initialized")
        logger.info(f"Using LLM: {self.llm_client.get_model_info()}")
        logger.info(f"Search tools: TavilyNewsAgency (supports 6 search tools)")
    ...
    def execute_search_tool(self, tool_name: str, query: str, **kwargs) -> TavilyResponse:
        ...
        Args:
            tool_name: Tool name, available options:
                ...
                - "search_news_last_week": News from this week
    ...
    def _initial_search_and_summary(self, paragraph_index: int):
        ...
        if search_tool == "search_news_by_date":
            ...
            if start_date and end_date:
                ...
                if self._validate_date_format(start_date) and self._validate_date_format(end_date):
                    ...
                    logger.info(f"  ⚠️  Invalid date format (should be YYYY-MM-DD), falling back to basic search")
                    logger.info(f"      Provided dates: start_date={start_date}, end_date={end_date}")
                    search_tool = "basic_search_news"
            else:
                logger.info(f"  ⚠️  search_news_by_date tool missing time parameters, falling back to basic search")
                search_tool = "basic_search_news"
        ...
        if search_response and search_response.results:
            # Each search tool has its specific result count, take first 10 as upper limit
    ...
    # =========================================================================
    # Query Agent New Interface (Phase 1+)
    # =========================================================================

    @property
    def query_graph(self):
        """Lazy initialization of LangGraph subgraph to avoid triggering during import."""
        if self._query_graph is None:
            from .graph.builder import build_query_agent_graph
            self._query_graph = build_query_agent_graph()
            logger.info("[QueryAgent] LangGraph subgraph initialized")
        return self._query_graph

    async def research_structured(self, query: str) -> Dict:
        """
        New interface (async): Execute stance-aware search, return structured QueryAgentOutput.

        Args:
            query: User query (Chinese or English)

        Returns:
            QueryAgentOutput dictionary, containing stance_distribution, sources,
            coverage_score, and other fields.
        """
        initial_state = {
    ...
    def research_structured_sync(self, query: str) -> Dict:
        """
        New interface (sync): Wrapper for research_structured(), compatible with non-async environments.
        """
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # Already in async context, use nest_asyncio or new thread
    ...
    def _write_forum_finding(self, output: Dict):
        """Write research findings to ForumEngine (via logs/query.log)."""
        forum_logger = logging.getLogger("query_engine")
        dist = output.get("stance_distribution", {})
        forum_logger.info(
            f"[FINDING] Stance distribution: "
            f"Support {dist.get('support', 0):.0%} / "
            f"Oppose {dist.get('oppose', 0):.0%} / "
            f"Neutral {dist.get('neutral', 0):.0%} / "
            f"Official {dist.get('official', 0):.0%}"
        )
        for gap in output.get("knowledge_gaps", [])[:3]:
            forum_logger.info(f"[REFLECTION] Knowledge gap: {gap}")
