...
class LLMClient:
    ...
    def __init__(self, api_key: str, model_name: str, base_url: Optional[str] = None):
        ...
        try:
            ...
            self.short_timeout, self.long_timeout = resolve_llm_timeouts(
                settings, "MEDIA_ENGINE_REQUEST_TIMEOUT"
            )
            self.stream_idle_timeout = resolve_stream_idle_timeout(settings)
        except Exception:
            self.short_timeout, self.long_timeout = 120.0, 600.0
    ...
    def stream_invoke(self, system_prompt: str, user_prompt: str, **kwargs) -> Generator[str, None, None]:
        """
        Stream invoke LLM, return response content incrementally
    ...
    def stream_invoke_to_string(self, system_prompt: str, user_prompt: str, **kwargs) -> str:
        """
        Stream invoke LLM and safely concatenate to complete string (avoid UTF-8 multi-byte character truncation)
        ...
        # Join all bytes, then decode at once
        if byte_chunks:
