...
import { percentText, compactNumber, durationText, signalGraphSummary } from '../utils/helpers';
  ...
  return (
    <motion.section key="command" className="home-stage" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
      ...
      <div className="metric-orbit">
        ...
        <MiniMetric icon={<SearchOutlined />} label="Proof" value={compactNumber(graphSummary.canonical_count || queryAgent.canonical_sources || queryAgent.total_sources)} onClick={() => setActive('evidence')} />
