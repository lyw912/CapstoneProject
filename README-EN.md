...
**Multimodal Agent** Multimodal content understanding: video, image, and structured information cards (`MediaEngine/`).

**Query Agent** Stance-aware breadth retrieval and evidence integration (`QueryEngine/`).

**Agent Coordinator** Forum debate orchestration for Multimodal and Query agents (`ForumEngine/`; LLM env prefix `FORUM_HOST_*`).

**Report Agent** Structured composite report generation (`ReportEngine/`).
...
| 2 | Parallel Launch | Two analysis agents start in parallel | Query Agent, Multimodal Agent | - |
...
| 5.1 | In-depth Research | Each agent conducts specialized search guided by the Agent Coordinator | Each Agent + Reflection Mechanisms + Coordinator Guidance | Each cycle |
| 5.2 | Forum Collaboration | ForumEngine monitors communications; Agent Coordinator (LLM) issues coordination guidance | ForumEngine + Agent Coordinator | Each cycle |
...
│   ├── prompts/                            # Prompt templates
│   └── ...
├── ReportEngine/                           # Report Agent: multi-round report generation
...
│   │   └── ...
│   └── __init__.py
├── ForumEngine/                            # Agent Coordinator: forum collaboration mechanism
│   ├── monitor.py                          # Log monitoring and forum management core
│   ├── llm_host.py                         # Agent Coordinator (LLM host) module
│   └── __init__.py
...
│   ├── query_engine_streamlit_app.py       # Query Agent (QueryEngine) standalone app
│   └── media_engine_streamlit_app.py       # Multimodal Agent (MediaEngine) standalone app
...
├── report_engine_only.py                   # Report Agent CLI entry (ReportEngine)
...
The main Web workflow (Multimodal / Query / Agent Coordinator / Report Agent) does **not** require a local database. Configure PostgreSQL/MySQL only when using Docker Compose database services or MindSpider crawler ingestion.

When you need a database, configure the connection as follows (MySQL field names shown; adjust for PostgreSQL as needed):
...
All LLM calls use the OpenAI-compatible API format. Configure **Multimodal / Query / Report / Agent Coordinator** keys (`MEDIA_ENGINE_*`, `QUERY_ENGINE_*`, `REPORT_ENGINE_*`, `FORUM_HOST_*`) plus your chosen search tool keys so the system can reach your providers.
...
- **Database**: Only if using MindSpider ingestion or Docker DB (PostgreSQL / MySQL, etc.)
...
### 5. Configure LLM (and optional database)

Copy the `.env.example` file in the project root directory and rename it to `.env`.

Edit the `.env` file and fill in your API keys (see `.env.example` or `config.py` for full lists). **Minimum** for `python app.py`: Multimodal / Query / Report / Agent Coordinator LLM variables and search-tool keys. **`DB_*` is optional** unless you use MindSpider or an external DB.

```yml
# ====================== LLM (examples; see .env.example for all keys) ======================
# Multimodal Agent (MEDIA_ENGINE_*)
MEDIA_ENGINE_API_KEY=
MEDIA_ENGINE_BASE_URL=
MEDIA_ENGINE_MODEL_NAME=

# Query Agent (QUERY_ENGINE_*)
QUERY_ENGINE_API_KEY=
QUERY_ENGINE_BASE_URL=
QUERY_ENGINE_MODEL_NAME=

# Report Agent (REPORT_ENGINE_*)
REPORT_ENGINE_API_KEY=
REPORT_ENGINE_BASE_URL=
REPORT_ENGINE_MODEL_NAME=

# Agent Coordinator (FORUM_HOST_*)
FORUM_HOST_API_KEY=
FORUM_HOST_BASE_URL=
FORUM_HOST_MODEL_NAME=

# Optional: database for MindSpider / Docker DB only
# DB_DIALECT=
# DB_HOST=
```
...
# Start MediaEngine (Multimodal Agent UI)
...
This tool bypasses the Multimodal / Query Streamlit agents, reads the latest reports from their output folders, and generates a consolidated report without the Web UI (skipping incremental file validation). Markdown after PDF is on by default (CLI flag to disable). Use it for quick retries or when debugging the Report Agent (ReportEngine).
...
2. **Get Latest Files**: Automatically retrieves the latest analysis reports from `media_engine_streamlit_reports` and `query_engine_streamlit_reports`
...
4. **Direct Report Generation**: Skips file addition verification and directly invokes Report Agent (ReportEngine) to generate comprehensive reports
...
- Ensure at least one of the two agent report directories contains `.md` report files (both recommended for richer synthesis)
...
```python
# Example: SentimentAnalysisModel integration (paths vary by submodule)
SENTIMENT_CONFIG = {
...
```python
# e.g. your_package/tools/custom_db_tool.py
...
```python
# Wire custom tools in your agent entry module
