...
Responsible only for news data and topic analysis storage and queries
"""
...
class DatabaseManager:
    ...
    def save_daily_news(self, news_data: List[Dict], crawl_date: date = None) -> int:
        """
        Save daily news data, overwrite if data already exists for the day
        ...
        try:
            saved_count = 0
            # Delete first in independent transaction to prevent cleanup failure if subsequent inserts fail
            with self.engine.begin() as conn:
                deleted = conn.execute(text("DELETE FROM daily_news WHERE crawl_date = :d"), {"d": crawl_date}).rowcount
                if deleted and deleted > 0:
                    logger.info(f"Overwrite mode: deleted {deleted} existing news records for the day")

            # Insert one by one, single failure doesn't affect subsequent inserts (each in independent transaction)
            for news_item in news_data:
                try:
                    # news_item.get('id') is already the complete news_id (format: source_item_id)
                    # To support the same news appearing on different dates, add crawl_date to news_id
