...
Integrates the complete workflow and command-line tools for topic extraction
"""
...
try:
    ...
    logger.error("Please ensure running from project root directory and all dependencies are installed")
    sys.exit(1)
...
class BroadTopicExtraction:
    ...
    async def run_daily_extraction(self,
        ...
        try:
            ...
            if not news_result['success'] or not news_result['news_list']:
                raise Exception("News collection failed or no news retrieved")
    ...
    def print_extraction_results(self, extraction_result: Dict):
        ...
        extraction_result_message += f"\n📰 News collected: {news_data.get('total_news', 0)} items\n"
        ...
        async with BroadTopicExtraction() as extractor:
            ...
            if result['success']:
                if show_details:
                    ...
                    logger.info(f"   News collected: {news_data.get('total_news', 0)} items")
                    logger.info(f"   Keywords extracted: {len(topic_data.get('keywords', []))} items")
                    logger.info(f"   Summary generated: {len(topic_data.get('summary', ''))} characters")
...
def main():
    ...
    if args.keywords < 1 or args.keywords > 200:
        logger.error("Keywords count should be between 1-200")
        sys.exit(1)
