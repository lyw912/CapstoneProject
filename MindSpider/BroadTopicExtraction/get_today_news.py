...
Integrates news API calls and database storage functionality
"""
...
class NewsCollector:
        ...
        for source in sources:
            ...
            if result["status"] == "success":
                ...
                if 'items' in data and isinstance(data['items'], list):
                    ...
                    logger.info(f"✓ {source_name}: Fetch successful, {count} news items")
                else:
                    logger.info(f"✓ {source_name}: Fetch successful")
            else:
    ...
    async def collect_and_save_news(self, sources: Optional[List[str]] = None) -> Dict:
        ...
        collection_summary_message += "\nStarting to collect daily trending news...\n"
...
async def main():
    ...
    async with NewsCollector() as collector:
        ...
        result = await collector.collect_and_save_news(
            sources=["weibo", "zhihu"]  # For testing, use only two sources
        )
        
        if result['success']:
            logger.info(f"Collection successful! Got {result['total_news']} news items")
        else:
