...
def generate_report(reports: list[str], query: str, pdf_available: bool) -> Dict[str, Any]:
    """
    调用 Report Agent（ReportEngine）生成报告
    ...
    try:
        ...
        # 初始化Report Agent
        logger.info("正在初始化 Report Agent（ReportEngine）...")
        agent = ReportAgent()

        # 定义流式事件处理器
        def stream_handler(event_type: str, payload: Dict[str, Any]):
            """处理 Report Agent 的流式事件"""
            if event_type == 'stage':
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Report Agent 命令行入口 - 无需前端的报告生成工具（ReportEngine）",
...
def main():
    ...
    logger.info("\n")
    logger.info("╔" + "═" * 68 + "╗")
    logger.info("║" + " " * 18 + "Report Agent 命令行版本" + " " * 26 + "║")
    logger.info("╚" + "═" * 68 + "╝")
    logger.info("\n")
