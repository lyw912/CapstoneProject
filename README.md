...
**Multimodal Agent** 多模态内容理解：视频、图像与结构化信息卡片等跨模态分析（代码目录 `MediaEngine/`）

**Query Agent** 立场感知的广度检索与证据整合：国内外网页与结构化检索能力（代码目录 `QueryEngine/`）

**Agent Coordinator** 论坛辩论协调：引导 Multimodal / Query 双 Agent 协作与观点碰撞（实现于 `ForumEngine/`，LLM 配置项仍为 `FORUM_HOST_*`）

**Report Agent** 结构化综合报告生成：内置模板的多轮装订与渲染（代码目录 `ReportEngine/`）
...
| 2 | 并行启动 | 两个分析 Agent 同时开始工作 | Query Agent、Multimodal Agent | - |
| 3 | 初步分析 | 各Agent使用专属工具进行概览搜索 | 各Agent + 专属工具集 | - |
| 4 | 策略制定 | 基于初步结果制定分块研究策略 | 各Agent内部决策模块 | - |
| 5-N | **循环阶段** | **论坛协作 + 深度研究** | **ForumEngine + 所有Agent** | **多轮循环** |
| 5.1 | 深度研究 | 各 Agent 基于 Agent Coordinator 引导进行专项搜索 | 各Agent + 反思机制 + 协调者引导 | 每轮循环 |
| 5.2 | 论坛协作 | ForumEngine 监控发言并由 Agent Coordinator（LLM）生成协调意见 | ForumEngine + Agent Coordinator | 每轮循环 |
...
├── report_engine_only.py                   # Report Agent 命令行入口（调用 ReportEngine）
├── README.md                               # 中文说明文档
├── README-EN.md                            # 英文说明文档
...
请配置 **Multimodal / Query / Report / Agent Coordinator**（环境变量仍为 `MEDIA_ENGINE_*`、`QUERY_ENGINE_*`、`REPORT_ENGINE_*`、`FORUM_HOST_*`）等 LLM 密钥与模型参数（与 Web 界面「LLM 配置」或根目录 `.env` 一致），确保系统能连接您选择的大模型服务。

#### 数据库配置（可选）

主界面与 **Report Agent、Agent Coordinator、Multimodal / Query** 全流程**不依赖本地数据库**，仅需上述 LLM 与搜索相关配置即可运行。

若使用 **Docker Compose 中的数据库服务**、或需要**单独运行 MindSpider 爬虫**并向数据库写入数据，再按 `docker-compose.yml` 与 [MindSpider 使用说明](./MindSpider/README.md) 配置 `DB_*` 等连接信息（PostgreSQL 示例）：
...
完成 LLM 配置后，系统即可正常运行；仅在使用爬虫入库等场景时再补全数据库项。
...
- **数据库**: 仅在使用 MindSpider 爬虫入库或 Docker 数据库服务时需要（PostgreSQL / MySQL 等）
- **内存**: 建议2GB以上
...
编辑 `.env` 文件，填入您的 API 密钥（也可自选模型、搜索代理，详情见根目录 `.env.example` 或 `config.py`）。

**最小运行（推荐）**：仅配置 **Multimodal / Query / Report / Agent Coordinator** 等与分析、报告相关的 LLM 变量，以及 **Tavily / Bocha / Anspire** 等搜索工具密钥（按你选用的搜索方式）。**无需填写 `DB_*`**，即可启动 `python app.py` 使用完整 Web 流程。

若需**单独运行 MindSpider** 并向数据库写入数据，再补充 `DB_DIALECT`、`DB_HOST`、`DB_PORT`、`DB_USER`、`DB_PASSWORD`、`DB_NAME`、`DB_CHARSET` 等（见 [MindSpider 使用说明](./MindSpider/README.md)）。

示例（仅作结构参考，键名以 `.env.example` 为准）：

```yml
# ====================== LLM 配置（示例）======================
# 您可以更改每个部分 LLM 使用的 API，只要兼容 OpenAI 请求格式即可
...
# 可选：仅在使用 MindSpider 等需要数据库的功能时填写
# DB_DIALECT=
# DB_HOST=
# ...
```
...
该工具会跳过 Multimodal / Query 两个分析 Agent（Streamlit）的运行阶段，直接读取它们输出目录中的最新报告文件，并在无需 Web 界面的情况下生成综合报告（同时省略文件增量校验步骤），默认会在 PDF 之后自动生成 Markdown（可用参数关闭）。通常用于对报告生成结果不满意、需要快速重试的场景，或在调试 Report Agent（ReportEngine）时启用。
...
2. **获取最新文件**：自动从两个引擎目录（`media_engine_streamlit_reports`、`query_engine_streamlit_reports`）获取最新的分析报告
3. **文件确认**：显示所有选择的文件名、路径和修改时间，等待用户确认（默认输入 `y` 继续，输入 `n` 退出）
4. **直接生成报告**：跳过文件增加审核程序，直接调用 Report Agent（ReportEngine）生成综合报告
5. **自动保存文件**：
...
- 确保 `media_engine_streamlit_reports` 与 `query_engine_streamlit_reports` 中至少有一个目录包含 `.md` 报告文件（二者最好都有，便于综合报告引用）
...
> 说明：原 **InsightEngine**（私有库挖掘）已从本仓库移除；情感分析与爬虫相关能力见 `SentimentAnalysisModel/`、`MindSpider/` 等目录。

#### 情感分析模型配置

```python
# SentimentAnalysisModel/ 下各模型目录中的 predict 配置（示例）
SENTIMENT_CONFIG = {
...
若需将**自有业务库**与舆情分析打通，可在项目根目录 `config.py` / `.env` 中增加自定义字段（如 `BUSINESS_DB_HOST` 等），并在 **QueryEngine / MediaEngine** 的 `tools/` 中实现查询工具，再在对应 `agent.py` 中注册为可调用的 Tool。

原 upstream 文档中基于 **InsightEngine** 的示例路径已不适用；本仓库已移除 InsightEngine，请按上述方式在现有引擎内扩展，或参考 `MindSpider/schema/` 与爬虫模块的数据库访问方式自行对接。
