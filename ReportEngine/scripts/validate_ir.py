...
- Scanning all charts and tables in specified JSON files
- Reporting structural issues and data missing
- Supporting automatic fixing of common issues
...
class IRValidator:
    ...
    def _validate_blocks(
        ...
        """Recursively validate blocks list"""
    ...
    def _validate_wordcloud(
        ...
        # Check various possible word cloud data paths
        data_paths = [
        ...
        for path_name, value in data_paths:
            if isinstance(value, list) and len(value) > 0:
                ...
                for idx, item in enumerate(value[:5]):  # Only check first 5
                    if isinstance(item, dict):
                        ...
                        warnings.append(f"{path_name}[{idx}] incorrect format")

                break

        if not words_found:
            errors.append("Word cloud data missing: no valid data found at data.words, data.items, props.words paths")
        elif words_count == 0:
        ...
        if errors or warnings:
            issue = BlockIssue(
                ...
                errors=errors,
                warnings=warnings,
                is_fixable=False,  # Word cloud data missing usually cannot be auto-fixed
            )
            report.issues.append(issue)
    ...
    def repair_document(
        ...
        Returns:
            Tuple[Dict[str, Any], int]: (Repaired document, fix count)
        """
        fixed_count = 0
        ...
        for block in blocks:
            ...
            if block_type == "table":
                ...
                if result.has_changes():
                    ...
                    fixed_count += 1
                    logger.info(f"Repaired table: {result.changes}")
            ...
            # Repair chart
                ...
                if "chart.js" in widget_type:
                    ...
                    if result.has_changes():
                        block = result.repaired_block
                        fixed_count += 1
                        logger.info(f"Repaired chart: {result.changes}")
...
def print_report(report: DocumentReport, verbose: bool = False):
    ...
    print(f"  - Chart count: {report.chart_count}")
