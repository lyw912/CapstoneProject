#!/usr/bin/env python3
"""
PDF Export Tool - Generate PDF directly using Python, no garbled text
...
def export_to_pdf(ir_json_path: str, output_pdf_path: str = None):
    ...
    Args:
        ...
        output_pdf_path: Output PDF path (optional, defaults to same name with .pdf extension)
    """
    ir_path = Path(ir_json_path)

    if not ir_path.exists():
        logger.error(f"File not found: {ir_path}")
        return False
...
def main():
    ...
    if 'DYLD_LIBRARY_PATH' not in os.environ:
        logger.warning("DYLD_LIBRARY_PATH not set, attempting automatic configuration...")
