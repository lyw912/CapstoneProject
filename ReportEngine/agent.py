...
1. 管理输入数据与状态，协调 Media/Query 分析引擎、论坛日志与模板；
...
class FileCountBaseline:
    ...
    该工具用于：
    - 在任务启动时记录 Media/Query 两个引擎导出的 Markdown 数量；
    - 在后续轮询中快速判断是否有新报告落地；
...
class ReportAgent:
    ...
    def _setup_logging(self):
        ...
        def _exclude_other_engines(record):
            """
            过滤掉其他引擎(Media/Query/Forum)产生的日志，其余日志全部保留。
    ...
    def _initialize_file_baseline(self):
        ...
        将 Media/Query 两个目录传入 `FileCountBaseline`，
        生成一次性的参考值，之后按增量判断两引擎是否产出新报告。
        """
        directories = {
    ...
    def _initialize_rescue_llms(self) -> List[Tuple[str, LLMClient]]:
        ...
        顺序遵循“Report → Forum → Media”，缺失配置会被自动跳过。
        """
    ...
    def generate_report(self, query: str, reports: List[Any], forum_logs: str = "",
        ...
        参数:
            ...
            reports: 来自 Query/Media 等分析引擎的原始输出，允许传入字符串或更复杂的对象。
    ...
    def _build_generation_context(
        ...
        参数:
            query: 用户查询词。
            reports: 归一化后的 query/media 报告映射。
            forum_logs: 引擎讨论记录。
    ...
    def _normalize_reports(self, reports: List[Any]) -> Dict[str, str]:
        ...
        返回:
            dict: 包含 `query_engine`/`media_engine` 两个字符串字段的映射。
        """
