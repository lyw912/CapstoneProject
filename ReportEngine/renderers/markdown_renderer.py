...
class MarkdownRenderer:
    ...
    - Charts and word clouds are uniformly degraded to data tables to avoid losing key information;
    - Preserve common features as much as possible (headings, lists, code, tables, quotes, etc.);
    ...
    def render(
        ...
        """
        Entry point: Convert IR to Markdown string.

        Parameters:
            document_ir: Document IR data
            ir_file_path: Optional, IR file path, if provided, fixes will be auto-saved
        ...
        # Although charts are degraded to tables during Markdown rendering, data validity still needs to be ensured
        # review_document returns session statistics (thread-safe, not used here)
    ...
    def _render_chapter(self, chapter: Dict[str, Any]) -> str:
        ...
        # Chapter titles use level 1 heading format, and avoid duplication with first heading
        if title:
    ...
    def _render_heading(self, block: Dict[str, Any]) -> str:
        ...
        # Insert extra blank lines before level 1 heading within chapter (doesn't affect document title)
        if level == 1:
            return f"\n\n\n\n\n{heading_line}"
        return heading_line
    ...
    def _is_metadata_paragraph(self, inlines: List[Any]) -> bool:
        """
        Detect if paragraph contains only document metadata JSON.
        
        Some LLM-generated content incorrectly outputs metadata (like xrefs, widgets, footnotes, metadata)
        as paragraph content. This method identifies and marks such cases for skipping rendering.
        """
    ...
    def _flatten_nested_cells(self, cells: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        In some LLM-generated table data, cells are incorrectly recursively nested:
        cells[0] normal, cells[1].cells[0] normal, cells[1].cells[1].cells[0] normal...
        This method flattens such nested structures into a standard parallel cell array.
    ...
    def _fix_nested_table_rows(self, rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        In some LLM-generated table data, all row cells are nested within the first row,
        resulting in a table with only 1 row containing all data. This method detects and fixes this.

        Parameters:
            rows: Original table row array.

        Returns:
            List[Dict]: Fixed table row array.
        """
        if not rows or len(rows) != 1:
            # Only handle the abnormal case with just 1 row
            return rows
        ...
        # First filter out placeholder cells
        ...
        def _is_header_cell(cell: Dict[str, Any]) -> bool:
            ...
            header_keywords = {
                ...
                "Platform", "Channel", "Source", "Description", "Note", "Remark", "Serial", "Number",
                "Event", "Key", "Data", "Support", "Reaction", "Market", "Sentiment", "Node",
                "Dimension", "Point", "Detail", "Label", "Impact", "Trend", "Weight", "Category",
                "Info", "Content", "Style", "Preference", "Main", "User", "Core", "Feature",
                "Classification", "Range", "Object", "Project", "Phase", "Cycle", "Frequency", "Level",
            }
        ...
        for cell in all_cells:
            if _is_header_cell(cell):
                ...
                # Encounter first non-header cell, data area begins
                break
        ...
        if remainder > 0 and remainder <= 3:
            ...
            # Remainder too large, column count detection may be wrong, return original data
            return rows
    ...
    def _render_table(self, block: Dict[str, Any]) -> str:
        ...
        if has_header and first_row_cells:
            ...
            header_cells = [f"Col{idx + 1}" for idx in range(col_count or (len(first_row_cells or []) or 1))]
    ...
    def _render_widget(self, block: Dict[str, Any]) -> str:
        ...
        try:
            ...
            data_preview = ""
        note = "> Data components not supported in Markdown rendering"
    ...
    def _render_wordcloud_as_table(self, block: Dict[str, Any]) -> str:
        ...
        lines = [
            self._markdown_row(["Keyword", "Weight", "Category"]),
    ...
    def _render_inline_run(self, run: Any, for_table: bool = False) -> str:
        if isinstance(run, dict):
            # Handle inlineRun type: nested inlines array
        ...
        # Try to detect and parse inlineRun JSON that was incorrectly serialized as a string
        ...
        for mark in marks:
            ...
            # Color/font and other non-generic marks are directly degraded to plain text
        return result

    def _apply_mark(self, text: str, mark: Any) -> str:
        """
        Apply single mark format to text.
        
        Used for processing outer marks of inlineRun type.
        """
        if not isinstance(mark, dict):
            return text
        mtype = mark.get("type")
    ...
    def _try_parse_inline_run_string(self, text: str) -> dict | None:
        """
        Try to parse inlineRun JSON that was incorrectly serialized as a string.
        
        Some LLM-generated content may accidentally store inlineRun structures as strings
        in the text field. This method attempts to identify and parse such cases.
        
        Parameters:
            text: String that may contain JSON
            
        Returns:
            dict | None: Returns inlineRun dictionary if parsing succeeds, otherwise None
        """
    ...
    def _is_heading_duplicate(self, block: Dict[str, Any], chapter_title: str | None) -> bool:
        """Determine if first heading duplicates chapter title"""
    ...
    def _normalize_heading_text(self, text: Any) -> str:
        """Remove serial number prefix and normalize whitespace"""
        if not isinstance(text, str):
            return ""
        stripped = text.strip()
        # Remove prefixes like "1.", "1.1", "一、" (Chinese numerals)
        for sep in (" ", "、"):
            if sep in stripped:
    ...
    def _looks_like_prefix(token: str) -> bool:
        """Determine if token looks like a serial number prefix"""
        if not token:
            return False
        if token.isdigit():
            return True
        chinese_numerals = set("一二三四五六七八九十零〇壹贰叁肆伍陆柒捌玖拾")  # Chinese numerals
    ...
    def _stringify_value(self, value: Any) -> str:
        ...
        if isinstance(value, dict):
            # Priority to common numeric fields
            for key in ("y", "value"):
                if key in value:
    ...
    def _fallback_unknown(self, block: Dict[str, Any]) -> str:
        try:
            ...
            payload = str(block)
        logger.debug(f"Unrecognized block type, using JSON fallback: {block}")
        return f"```json\n{payload}\n```"
