...
class LSTMModel(BaseModel):
    ...
    def _train_word2vec(self, train_data: List[Tuple[str, int]], **kwargs):
        ...
        print(f"Word2Vec training completed, embedding dimension: {vector_size}")
    ...
    def predict(self, texts: List[str]) -> List[int]:
        ...
        # Create dataset
        test_data = [(text, 0) for text in texts]  # Labels are irrelevant
    ...
    def predict_single(self, text: str) -> Tuple[int, float]:
        ...
        if len(vectors) == 0:
            return 0, 0.5  # Return default values if no valid word vectors
    ...
    def save_model(self, model_path: str = None) -> None:
        """Save model"""
        if not self.is_trained:
            raise ValueError(f"Model {self.model_name} is not trained yet, cannot save")
