...
try:
    ...
    gpu_to_use = "0"
# If not set or multi-GPU is exposed, force single GPU exposure (default 0) for stable direct execution
if (not env_vis) or ("," in env_vis):
    os.environ["CUDA_VISIBLE_DEVICES"] = gpu_to_use
os.environ.setdefault("CUDA_DEVICE_ORDER", "PCI_BUS_ID")

# Clean up distributed environment variables that may be injected by external launchers to avoid accidental multi-GPU/distributed mode
...
# Pre-configured optional Chinese backbone models (extensible)
...
def prompt_backbone_interactive(current_id: str) -> str:
    """Interactive backbone model selection.

    - Returns current_id directly when in non-interactive environment (stdin not TTY) or when NON_INTERACTIVE=1 is set.
    - Users can input a number to select from preset options, or directly input any Hugging Face model ID.
    ...
    print("\nAvailable Chinese backbone models (press Enter to use default):")
    for label, hf_id in BACKBONE_CANDIDATES:
        print(f"  {label}")
    print(f"Current default: {current_id}")
    choice = input("Please enter number or paste model ID (press Enter to use default): ").strip()
    if not choice:
        return current_id
    # Number option
    if choice.isdigit():
        ...
        for label, hf_id in BACKBONE_CANDIDATES:
            if label.startswith(f"{idx})"):
                return hf_id
        print("Number not found, using default.")
        return current_id
    # Custom HF model ID
    return choice
...
def preprocess_text(text: str) -> str:
    # Basic text cleaning: remove extra whitespace and normalize line breaks
    text = str(text).strip()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eval_fraction", type=float, default=0.25, help="Evaluate and save every fraction of an epoch, e.g., 0.25 means every quarter epoch")
    parser.add_argument("--early_stop_patience", type=int, default=5, help="Early stopping patience (counted by evaluation rounds)")
...
def main() -> None:
    ...
    # Interactive backbone model selection (if interactive and not disabled by environment)
    ...
    if not os.path.isfile(valid_path):
        # If validation set is not provided or doesn't exist, auto-split
    ...
    # Label mapping (use union of training set and validation set to avoid new labels in validation set causing errors)
    ...
    # Warn about labels in validation set not present in training set
    try:
    ...
    # Optional arguments (only add if supported, simplify to match reference implementation for better compatibility)
    if "warmup_ratio" in allowed:
        args_dict["warmup_ratio"] = args.warmup_ratio
    if "report_to" in allowed:
        args_dict["report_to"] = []
    # Evaluation/save steps: calculate steps per epoch based on eval_fraction
    ...
    # Strategy-based configuration (compatible with both new and old field names)
    ...
    # Align logging steps with evaluation/save steps to reduce output verbosity
    if "logging_steps" in allowed:
        args_dict["logging_steps"] = eval_every_steps
    # Best model checkpoint loading (enabled only when evaluation and save strategies match)
    ...
    # Compatibility for versions without warmup_ratio: use warmup_steps if available, skip ratio
    ...
    # Fallback for versions without strategy parameters: save/evaluate every eval_every_steps
    ...
    # If load_best_model_at_end is supported but evaluation/save strategies cannot be set simultaneously, disable it to avoid errors
    if "load_best_model_at_end" in allowed:
